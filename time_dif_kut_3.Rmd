...
cv_stats_38 <- processed_data %>%
  # BIZTONSÁGOS SZŰRÉS:
  # Nem a nevet ("Rövid") keressük, hanem a konkrét frame számokat (38, 39).
  # Ez kikerüli az ékezetes betűk (ö) okozta hibákat.
  filter(ref_frame %in% c(38, 39)) %>%
  filter(group %in% c("normal", "sz_hr")) %>%
  
  # Csoportnevek tisztítása (Biztosítjuk, hogy csak 2 csoport legyen)
  ...
  # Kiszűrjük, ha valakinek 0 vagy végtelen lett a CV-je (pl. csak 1 mérése volt)
...
# Csak akkor futtatjuk, ha mindkét csoportban van adat
...
# Aggregáljuk az adatokat: Minden ID-hoz és Ingerhosszhoz kiszámoljuk a CV-t
